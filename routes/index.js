...
router.post('/', upload.single('avatar'), function(req, res, next) {
  ...
  const webWordBank = webDetect(req.file['path'])
    .then(results => {
      webWords = results;
      labelDetect(req.file['path'])
        .then(labelResults => {
          labelWords = labelResults;
          const combinedWordBank = sortBySyllable([
            ...new Set(results['arrayOfWords'].concat(labelResults))
          ]);
          ...
          let i = 0;
          while (
            labelResults[i].indexOf(' ') != -1 &&
            i < labelResults.length
          ) {
            i++;
          }
        ...
        .then(wordBank => {
          ...
          const adjectiveOfBestWordObjects = makeRequest(
            'GET',
            `https://api.datamuse.com/words?rel_jjb=${
              wordBank['wordForAdjective']
            }`
          )
            .then(function(result) {
              const arrayOfAdjectives = sortBySyllable(
                JSON.parse(result).map(wordObject => {
                  return wordObject['word'];
                })
              );

              const mySentence1 = generateSentence(
                wordBank['combinedWordBank'],
                5,
                arrayOfAdjectives
              );
              const mySentence2 = generateSentence(
                wordBank['combinedWordBank'],
                7,
                arrayOfAdjectives
              );
              const mySentence3 = generateSentence(
                wordBank['combinedWordBank'],
                5,
                arrayOfAdjectives
              );
...
function generateSentence(words, syllables, adjectives) {
  ...
  // currently removing this function because it is causing a lot of undefines to return, will improve it shortly
  // const firstWordIndex =
  //   words[firstWordSyllables] && words[firstWordSyllables].indexOf(firstWord);
...
function webDetect(imagePath) {
  return client.webDetection(imagePath).then(results => {
    ...
    if (bestGuessLabels[0].label.indexOf(' ') === -1) {
      ...
      webWordBank['bestGuess'] =
        bestGuessWordSplit[bestGuessWordSplit.length - 1];
    }
